...
update_user_payload = {"Authorization": f"Bearer {response_login_data['token']['accessToken']}"}
...
update_user_response = httpx.patch(f"http://localhost:8000/api/v1/users/{create_user_response_data['user']['id']}", json= body_update_user, headers=update_user_payload)
